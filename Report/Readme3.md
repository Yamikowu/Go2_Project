...
### 💡 為什麼 ONNX + TensorRT 是關鍵？

我們選模型時特別看重「**ONNX 支援**」，這也是未來挑選其他 AI 模型時的重要標準。

**ONNX (Open Neural Network Exchange)**

- AI 模型的「**通用翻譯格式**」。不同框架 (PyTorch, TensorFlow) 訓練出的模型，轉成 ONNX 後就能在任何平台執行。
- 好處：我們在 Mac 上開發的程式碼，**不用改一行**就能搬到 Jetson 上跑。

**TensorRT (NVIDIA 專屬加速引擎)**

- NVIDIA 針對自家 GPU 開發的「**編譯級優化工具**」，能把 ONNX 模型進一步壓縮、加速。
- 效果：推論速度可提升 **3~10 倍**，記憶體佔用大幅降低。
- 在 Jetson 上，不開 TensorRT 可能只有 1~2 FPS；**開了之後可以達到 30+ FPS**。

> **未來選模型的原則**：優先選擇支援 ONNX 匯出的方案，這樣才能在 Jetson 上透過 TensorRT 獲得最佳效能。
